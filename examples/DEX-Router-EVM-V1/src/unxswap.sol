...
contract SmartSwap {
    ...
    function performTokenSwap(
        ...
        for (uint i = 0; i < pools_.length; i++) {
            ...
            uint isToken0Tax = isToken0Tax[i] ? _IS_TOKEN0_TAX : 0;
            uint isToken1Tax = isToken1Tax[i] ? _IS_TOKEN1_TAX : 0;
            uint numerator = (numerator[i] << 160) & _NUMERATOR_MASK;
            uint isReverse = zeroForOne[i] ? 0 : _REVERSE_MASK;
            uint isReceiveEth = receiveEth ? _WETH_MASK : 0;
            uint pool = poolAddress |
                isToken0Tax |
                isToken1Tax |
                numerator |
                isReverse |
